...
int colGas = 10; // val lo = 0, hi = 8000; but my range appears to be much higher ~65000!
...
void setup() {
  ...
  lons = new FloatList();
  //
  table = loadTable("merged-air-quality.csv", "header");
...
void draw() {
  ...
  */
  ...
  for (int i=0; i<locs.length; i+=1) {
    ...
    float v = norm(table.getFloat(i,colGas),0,65000);
    float d = 2*sqrt(v/PI);
    fill(255,204,102);
