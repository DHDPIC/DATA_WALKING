...
void draw() {
  ...
  */
  ...
  // PLOT TEMP // note increment every 10 values
  for (int i=0; i<locs.length; i+=10) {
    ScreenPosition pos1 = map.getScreenPosition(locs[i]);
    
    float v = map(table.getFloat(i,colTemp),5,25,0,255);
    fill(v,v*0.5,255-v,51);
    ellipse(pos1.x, pos1.y, 16, 16);
  }
  if(render) {
    render = false;
    saveFrame("fr-####.tiff");
