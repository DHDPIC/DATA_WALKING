...
void setup() {
  ...
  interpToGrid = createWriter("interp-to-grid-polygon.geojson");
...
void draw() {
  ...
  if (render) {
    ...
    save("exports-p-"+inc+"-"+gsize+"/"+timestamp+"-amap.tiff");
  }
  // PLOT A ROUTE
  ...
  if (render) {
      ...
      for(int xx=0; xx<width; xx+=gsize) {
        ...
        for(int d=0; d<num; d+=1) {
          ...
          if(pos.x >= xx && pos.x < xx+gsize && pos.y >= yy && pos.y < yy+gsize) {
            //println("find " + d);
            float vd = realVals[d]; // this should be realVals[]!!!
            av += vd;
            mx = max(mx,vd);
            t++;
            tx += pos.x;
    ...
    save("exports-p-"+inc+"-"+gsize+"/"+timestamp+"-xpo.tiff");
    
    // export a new interpolated dataset
    for(int yy=0; yy<height; yy+=gsize) {
      for(int xx=0; xx<width; xx+=gsize) {
        // go through each grid square
        float gt = 0;
        float ginc = 0;
        for(int gy=yy; gy<yy+gsize; gy++) {
          for(int gx=xx; gx<xx+gsize; gx++) {
            color c = get(gx,gy);
            gt += brightness(c);
            ginc++;
          }
        ...
        gt /= ginc;
        println("gt = " + gt);
        float mapVal = map(gt,0,255,0,75);
        ...
        // for geojson writing
        interpToGrid.println("{");
        interpToGrid.println("\"type\": \"Feature\",");
        interpToGrid.println("\"geometry\": {");
        interpToGrid.println("\"type\": \"Polygon\",");
        // for point
        //valuesToGrid.println("\"coordinates\": ["+ gloc.getLon() +","+ gloc.getLat() +"]");
        // for polygon square
        interpToGrid.println("\"coordinates\": [[");
        interpToGrid.println("["+ brloc.getLon() +","+ brloc.getLat() +"],");
        interpToGrid.println("["+ trloc.getLon() +","+ trloc.getLat() +"],");
        interpToGrid.println("["+ tlloc.getLon() +","+ tlloc.getLat() +"],");
        interpToGrid.println("["+ blloc.getLon() +","+ blloc.getLat() +"],");
        interpToGrid.println("["+ brloc.getLon() +","+ brloc.getLat() +"]");
        interpToGrid.println("]]");
        // end of polygon
        interpToGrid.println("},");
        interpToGrid.println("\"properties\": {");
        interpToGrid.println("\"audio\":"+mapVal+"");
        interpToGrid.println("}");
        interpToGrid.println("},");
        //
      }
